...
sub compile {
	...
	my( $self, $template_name, $is_utf8 ) = @_ ;

	my $tmpl_ref = eval {
		 $self->_get_template( $template_name, $is_utf8 ) ;
	} ;
...
sub render {
	...
	my( $self, $template_name, $data, $is_utf8 ) = @_ ;
...
# assume the template name are is the actual template
		...
		$tmpl_ref =
			eval{ $self->_get_template( $template_name, $is_utf8 ) } ||
			\$template_name ;
	}
...
# not found, so find, slurp in and cache the template
	...
	$template = $self->_find_template( $tmpl_name, $is_utf8 ) ;
...
#warn "\$is_utf8 => $is_utf8";
		return read_file( $tmpl_path, ($is_utf8 ? (binmode => ':utf8') : ()), scalar_ref => 1 ) ;
	}
